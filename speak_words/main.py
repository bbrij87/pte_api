...
def generate_pronunciation(word, language_code=None, gender=None):
    ...
    logging.info(
        f"Generating pronunciation for word: {word} with language={language_code} and gender={gender}")
    ...
    audio_config = texttospeech.AudioConfig(
        audio_encoding=texttospeech.AudioEncoding[default_audio_encoding])
...
def process_sentence_pronunciation(sentence, language_code=None, gender=None):
    ...
    for word in words:
        ...
        if file_path:
            ...
            try:
                file_path = generate_pronunciation(
                    clean_word, language_code, gender)
                results[clean_word] = {
                    "status": "generated",
                    "path": str(file_path)
                }
                ...
                logging.error(
                    f"Error generating pronunciation for '{clean_word}': {e}")
                results[clean_word] = {
                    "status": "error",
                    "message": str(e)
                }
